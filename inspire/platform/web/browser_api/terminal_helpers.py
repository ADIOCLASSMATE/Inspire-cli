...
_log = logging.getLogger("inspire.platform.web.browser_api.terminal_helpers")
...
def _send_terminal_command_via_websocket(
    ...
    *page_or_frame* should be the Playwright frame whose origin matches the
    ...
    Waits for a shell prompt before sending stdin so that the command is
    not lost if bash hasn't initialized yet.
...
# ---------------------------------------------------------------------------
# Terminal DOM manipulation constants
# ---------------------------------------------------------------------------
...
def _focus_terminal_input(
    ...
    """Focus the xterm.js terminal input textarea."""
